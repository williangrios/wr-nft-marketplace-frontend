...
import { AiFillFile, AiFillFire, AiFillHeart, AiOutlineHeart } from 'react-icons/ai'
    ...
    return (
        <div className='w-[90%] lg:w-[80%] m-auto p-4 grid grid-cols-1 lg:grid-cols-2 items-center'>
            <div className="bg-gray-100 rounded-lg z-50 h-[90vh] max-h-[90vh] shadow-right-bottom p-8 text-gray-600 space-y-10">
                <h2 className="text-4xl font-extrabold">{sliderData[idNumber].title}</h2>
                <div className="flex gap-4 justify-between items-center">
                    <div className="flex w-1/2 gap-4">
                        <div className="">
                            <Image className='rounded-full shadow-right-bottom' src={sliderData[idNumber].image} alt='Profile image' width={50} height={50} />
                        </div>
                        <div className="flex flex-col gap-1">
                            <p className="">Creator</p>
                            <h4 className="flex gap-1">{sliderData[idNumber].name}<span><MdVerified /></span></h4>
                        </div>
                    </div>
                    <div className="flex w-1/2 gap-4">
                ...
                <div className="">
                    <div className="rounded-lg border-gray-400 border-2 border-opacity-35 mt-10">
                        <small className="bg-gray-200 rounded-lg p-5 ml-10">Current Bid</small>
                        <p className="px-10 mt-6 mb-3">{sliderData[idNumber].price}<span>$222,22</span></p>
                    </div>
                    ...
                    <div className="flex gap-6 mt-4 lg:mt-8 justify-start items-center">
                        <div className="flex flex-col items-center justify-center gap-3 w-[50px]">
                            <p className="text-4xl text-center font-extrabold">{sliderData[idNumber].time.days}</p>
                            <span className="text-center">Days</span>
                        </div>
                        <div className="flex flex-col items-center justify-center gap-3 w-[50px]">
                            <p className="text-4xl text-center font-extrabold">{sliderData[idNumber].time.hours}</p>
                            <span className="text-center">hours</span>
                        </div>
                        <div className="flex flex-col items-center justify-center gap-3 w-[50px]">
                            <p className="text-4xl text-center font-extrabold">{sliderData[idNumber].time.minutes}</p>
                            <span className="text-center">mins</span>
                        </div>
                        <div className="flex flex-col items-center justify-center gap-3 w-[50px]">
                            <p className="text-4xl text-center font-extrabold">{sliderData[idNumber].time.seconds}</p>
                ...
                <div className="flex mt-20 gap-8">
                    <TbArrowBigLeftLine className='rounded-full hover:bg-gray-300 w-8 h-8 cursor-pointer text-gray-500' onClick={() => decrement()} />
                    <TbArrowBigRightLine className='rounded-full hover:bg-gray-300 w-8 h-8 cursor-pointer text-gray-500' onClick={() => increment()} />
                </div>
            </div>
            <div className="relative bg-gray-200 mt-4 lg:mt-0 rounded-lg lg:ml-[-60px] h-[100vh] lg:max-h-[100vh] shadow-right-bottom">
                <Image src={sliderData[idNumber].nftImage} className='rounded-lg' alt='NFT Image' layout='fill' objectFit='cover' />
